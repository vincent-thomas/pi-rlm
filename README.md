...
Protected paths, including ignored files, are snapshotted outside the editable worktree before the first attempt. Rejected attempts restore preexisting ignored files under those paths and remove newly created ones without deleting ignored files elsewhere. The job directory must be outside the editable worktree. Large protected trees incur snapshot storage and hashing costs.
