...
Protected paths, including ignored files, are snapshotted outside the editable worktree before the first attempt. Protected trees containing symlinks (including ignored links) are rejected at startup: a link could alias mutable data outside protected paths. Rejected attempts restore preexisting ignored files under those paths and remove newly created ones without deleting ignored files elsewhere, provided the baseline is intact. Restoration refuses symlinked protected-path parents rather than risk writing outside the worktree; such a workspace needs manual inspection. The job directory must be outside the editable worktree. Large protected trees incur snapshot storage and hashing costs.

This is **not a security boundary against a malicious same-UID agent**. The agent has the user's filesystem permissions and can alter both the job directory's snapshot and manifest, or race filesystem checks; matching them to forged contents can fool validation and corrupt restoration. Keeping the job directory outside the worktree prevents incidental Git cleanup but does not make its data unforgeable. For adversarial agents, isolate their filesystem permissions (for example a separate user/container) and keep verifier inputs, baseline storage, and executable dependencies outside that isolation. Preserve independent backups of important ignored/user files before autonomous runs.

The verifier must be a trusted canonical executable at an absolute path outside the editable worktree, with no command-line arguments. This deliberately rejects workspace scripts and symlinks (even if they resolve outside the worktree), interpreter module flags, preloads, and inline commands: generic argument parsing cannot prove what code they load. The verifier and all of its executable dependencies must be immutable to the agent; protect benchmark/correctness inputs in the worktree and do not import unprotected workspace code as verifier logic. This is not a sandbox: an agent with access to the external verifier or its dependencies can forge scores. A detached supervisor PID is recorded for best-effort failure notification if it dies; a stale running state is also reported after its deadline plus a one-minute grace period. PID reuse and races remain possible. The verifier must be deterministic enough to compare runs and print exactly one JSON object:
